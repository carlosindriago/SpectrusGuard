...
// Check if cloaking is fully configured (for stepper completion)
$is_cloaking_enabled = !empty($settings['url_cloaking_enabled']);
?>
...
<div class="sg-gc-wizard">
    ...
    <div class="sg-gc-stepper">
        ...
        <div
            class="sg-gc-step <?php echo $step >= 3 ? 'active' : ''; ?> <?php echo $is_cloaking_enabled ? 'completed' : ''; ?>">
            <div class="sg-gc-step-circle">
                <?php echo $is_cloaking_enabled ? '✓' : '3'; ?>
            </div>
    ...
    <!-- REST API Stealth Panel -->
    <div class="sg-gc-panel" style="<?php echo !$can_activate ? 'opacity: 0.5; pointer-events: none;' : ''; ?>">
        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
            <div>
                <h3 style="margin: 0 0 8px 0; font-size: 18px; color: var(--sg-text-primary);">
                    🔐 <?php esc_html_e('REST API Stealth', 'spectrus-guard'); ?>
                </h3>
                <p style="color: var(--sg-text-secondary); margin: 0 0 20px 0;">
                    <?php esc_html_e('Hide the WordPress REST API endpoint from bots and scanners. When enabled, /wp-json/ returns 404.', 'spectrus-guard'); ?>
                </p>
            </div>
        ...
        <div class="sg-control-group" style="margin-bottom: 20px;">
            <div style="display: flex; align-items: center; justify-content: space-between; padding: 16px; background: var(--sg-bg-app); border-radius: 8px; border: 1px solid var(--sg-border);">
                <div>
                    <label class="sg-control-label" style="font-weight: 600; margin-bottom: 4px; display: block;">
                        <?php esc_html_e('Hide API Index', 'spectrus-guard'); ?>
                    </label>
                ...
                <label class="sg-switch">
                    <input type="checkbox" name="spectrus_shield_settings[api_hardening][hide_index]" value="1" <?php checked($hide_index); ?>>
        ...
        <div class="sg-control-group">
            <label class="sg-control-label" style="font-weight: 600; margin-bottom: 8px; display: block;">
                <?php esc_html_e('Custom API Prefix', 'spectrus-guard'); ?>
            </label>
            <p style="color: var(--sg-text-muted); margin: 0 0 12px 0; font-size: 13px;">
                <?php esc_html_e('Replace /wp-json/ with a custom path. Bots scanning for /wp-json/ will get 404.', 'spectrus-guard'); ?>
            </p>
            <div style="display: flex; gap: 12px; align-items: center;">
                <div style="color: var(--sg-text-muted);"><?php echo esc_html(home_url('/')); ?></div>
                <input type="text" 
                    name="spectrus_shield_settings[api_hardening][custom_prefix]" 
                    value="<?php echo esc_attr($custom_prefix); ?>"
                    placeholder="api/v1/secure"
                    style="flex: 1; max-width: 300px; background: var(--sg-bg-app); border: 1px solid var(--sg-border); color: var(--sg-text-primary); padding: 10px 14px; border-radius: 6px; font-family: monospace;">
                <div style="color: var(--sg-text-muted);">/</div>
            </div>
            <p style="color: var(--sg-text-secondary); margin: 12px 0 0 0; font-size: 12px;">
        ...
        <div class="sg-notice warning" style="margin-top: 20px;">
            <p style="margin: 0; font-size: 13px;">
                ⚠️ <strong><?php esc_html_e('Important:', 'spectrus-guard'); ?></strong>
